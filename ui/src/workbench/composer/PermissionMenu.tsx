...
import { formatEffort, permissionDesc, permissionLabel, t } from "../../lib/i18n";
...
export function subagentStateText(
  ...
  if (mode === "on") return t.subagentsStateOn;
  if (mode === "off") return t.subagentsStateOff;
  if (active) return t.subagentsStateAutoOn;
  return topEffort
    ? `${t.subagentsStateAutoOff}${t.shell.subagentsAutoOffAt(formatEffort(topEffort))}`
    : t.subagentsStateAutoOff;
}
...
export function PermissionMenu({
  ...
  return (
    ...
    >
      <div className="px-2.5 py-1.5 text-xs font-medium text-ink-2">{t.permTitle}</div>
      ...
      <div className="mt-1 border-t border-line pt-1">
        <div className="px-2.5 py-1.5 text-xs font-medium text-ink-2">{t.permProjectSection}</div>

        {/* IUCr upload opt-in */}
        <div className={rowCls} data-testid="settings-iucr">
          <span className="min-w-0 flex-1 truncate text-sm text-ink" title={t.iucrTip}>
            {t.settingsIucrTitle}
          </span>
          <Switch
            checked={iucrOn}
        ...
        <div className="rounded-lg px-2.5 py-1" data-testid="settings-subagents">
          <div className="flex h-6 items-center justify-between gap-3">
            <span className="min-w-0 flex-1 truncate text-sm text-ink" title={t.subagentsTip}>
              {t.subagentsSwitch}
            </span>
        ...
        <div className={rowCls} data-testid="settings-knowledge-mode">
          <span className="min-w-0 flex-1 truncate text-sm text-ink" title={t.knowledgeTip}>
            {t.settingsKnowledgeMode}
          </span>
          <select
            value={knowledgeMode}
            disabled={busy !== null}
            aria-label={t.settingsKnowledgeMode}
            title={t.knowledgeTip}
            onChange={(e) => void run("km", () => wb.changeKnowledgeMode(e.target.value))}
            className={selectCls}
          >
            {knowledgeModes.map((m) => (
              <option key={m} value={m}>
                {t.knowledgeModeLabels[m] ?? m}
              </option>
            ))}
          </select>
        </div>
        ...
        <div className={rowCls} data-testid="settings-structure-class">
          <span className="min-w-0 flex-1 truncate text-sm text-ink" title={t.structureClassTip}>
            {t.settingsStructureClass}
          </span>
          <select
            value={wb.structureClass ?? ""}
            disabled={busy !== null}
            aria-label={t.settingsStructureClass}
            title={t.structureClassTip}
            onChange={(e) => {
            ...
            className={selectCls}
          >
            <option value="">{t.settingsStructureClassNone}</option>
