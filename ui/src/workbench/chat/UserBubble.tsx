...
export function steerReceipt(item: UserItem): SteerReceipt | null {
  ...
  if (item.pending) return { state: "sending", label: zh.receiptSending };
  if (item.receipt === "failed") {
    return { state: "failed", label: zh.receiptFailed, title: item.receiptError };
  }
  if (item.receipt === "submitted") return { state: "submitted", label: zh.receiptSubmitted };
  return { state: "persisted", label: zh.receiptPersisted };
}
