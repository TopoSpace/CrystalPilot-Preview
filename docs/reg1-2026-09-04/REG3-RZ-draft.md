...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| rz | B/full | publication | 0.1032 | 0.0996 | +0.0036 | 是 | — | s2 CIF↔fcf:是<br>s3 REPORT↔CIF:是<br>s4 checkCIF解释门:是<br>e 窥视干净:是<br>自洽:是<br>结论↔CIF:是<br>未决披露:3 条 | 是（交付即最佳，或差距在 0.01 以内） | 13 min | 7,285,299 | 154 | 1 | — | `250395d3` 是 | graded |
...
### 3.1 rz-full-r1（B/full）

- rollout 记录 519 条；工具调用 154 次，其中出错 1 次；推理段 35 段

**工具错误率**

| 工具 | 调用 | 出错 | 错误率 | 累计耗时 | 最常见错误 |
|---|---|---|---|---|---|
| `checkout` | 3 | 1 | 0.333 | 0.3s | `Input validation error: 'node' is a required property` |
| `exec` | 90 | 0 | 0.0 | — | — |
| `run_shelxl` | 9 | 0 | 0.0 | 4.6s | — |
| `read_skill` | 5 | 0 | 0.0 | — | — |
| `run_shelxt` | 5 | 0 | 0.0 | 3.0s | — |
| `refine` | 4 | 0 | 0.0 | 1.6s | — |
| `view_structure` | 3 | 0 | 0.0 | 1.2s | — |
| `get_project_brief` | 2 | 0 | 0.0 | 0.3s | — |
| `ingest_vendor_data` | 2 | 0 | 0.0 | 0.4s | — |
| `audit_reflection_data` | 2 | 0 | 0.0 | 0.1s | — |
| `branch` | 2 | 0 | 0.0 | 0.4s | — |
| `situation_report` | 2 | 0 | 0.0 | 4.0s | — |
| `add_hydrogens` | 2 | 0 | 0.0 | 0.2s | — |
| `set_twin` | 2 | 0 | 0.0 | 0.1s | — |
| `validate_structure` | 2 | 0 | 0.0 | 0.1s | — |
| `finalize_delivery` | 2 | 0 | 0.0 | 0.1s | — |
| `set_experiment` | 1 | 0 | 0.0 | 0.2s | — |
| `screen_space_groups` | 1 | 0 | 0.0 | 0.3s | — |
| `change_space_group` | 1 | 0 | 0.0 | 0.1s | — |
| `inspect_model` | 1 | 0 | 0.0 | — | — |
...
| `write_outputs` | 1 | 0 | 0.0 | 0.2s | — |
| `run_checkcif` | 1 | 0 | 0.0 | 5.8s | — |
...
- `checkout` ×1　参数 `node`（missing_required）：`'node' is a required property`

**原地打转（短窗口内重复同一调用、同一参数）**

轮询已剔除（见下一小节）：detach 任务的状态查询本来就该重复。

- `run_shelxl` 重复 2 次　参数 `{"l_s": 20, "mode": "adopt"}`
- `get_project_brief` 重复 1 次　参数 `{}`
- `run_shelxl` 重复 1 次　参数 `{"l_s": 30, "mode": "adopt"}`
- `validate_structure` 重复 1 次　参数 `{}`

**轮询（detach 任务的状态查询，按工具契约每 60-120 s 一次）**

- `run_shelxt` 轮询 `job_20260904_143150` 1 次
- `run_shelxt` 轮询 `job_20260904_143242` 1 次
...
`read_skill` 4 次 4 种; `run_shelxt` 3 次 3 种

**转向（换打法，并附紧邻其前的工具错误）**

- 触发词 `unavailable`，其前无工具报错
  > …essing unresolved issues**  I need to create an unresolved list, focusing on the high R values from merged vendor data. The exact cause is still unclear due to unavailable classes and missing experimental metadata. The status of my outputs is provisional, and I need to establish an output directory
...
| `run_checkcif` | 1 | 5.8s |
| `run_shelxl` | 9 | 4.6s |
| `situation_report` | 2 | 4.0s |
| `run_shelxt` | 5 | 3.0s |
| `refine` | 4 | 1.6s |
| `view_structure` | 3 | 1.2s |
| `ingest_vendor_data` | 2 | 0.4s |
| `branch` | 2 | 0.4s |
| `checkout` | 3 | 0.3s |
| `get_project_brief` | 2 | 0.3s |
| `screen_space_groups` | 1 | 0.3s |
| `add_hydrogens` | 2 | 0.2s |
| `set_experiment` | 1 | 0.2s |
| `write_outputs` | 1 | 0.2s |
| `audit_reflection_data` | 2 | 0.1s |
...
- **rz-full-r1**
  - [schema 拒绝] `checkout` 的调用被 MCP 层挡下 1 次，参数 `node`（missing_required）：工具没跑过，是参数面的问题
...
|---|---|---|---|---|---|---|---|---|
| `run_shelxl` | 3 | 0 | 0 | 0 | 3 | 9 | 0 | 0.0 |
| `checkout` | 2 | 1 | 1 | 0 | 0 | 3 | 0 | 0.333 |
| `get_project_brief` | 1 | 0 | 0 | 0 | 1 | 2 | 0 | 0.0 |
| `validate_structure` | 1 | 0 | 0 | 0 | 1 | 2 | 0 | 0.0 |
...
|---|---|---|---|---|---|---|---|---|
| rz-full-r1 | gpt-5.6-sol | xhigh | full | <!-- crystalpilot-agents-v33 --> | `250395d3` | 是 | null（干净） | 1 |
