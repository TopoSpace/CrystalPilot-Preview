...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| nm | B/full | acceptable | 0.0948 | 0.0526 | +0.0006 | 是 | — | s2 CIF↔fcf:是<br>s3 REPORT↔CIF:是<br>s4 checkCIF解释门:是<br>e 窥视干净:是<br>自洽:是<br>结论↔CIF:是<br>未决披露:2 条 | 是（交付即最佳，或差距在 0.01 以内） | 12 min | 8,086,799 | 156 | 2 | — | `c92b3945` 是 | graded |
...
| 未达 publication 的原因 | — | - disorder incomplete: C15B, C16B of the reference are not accounted for - model the component(s) before publication<br>- metadata incomplete (A-level CIF-item alerts, not scored): 183, 184, 185, 197, 198, 699<br>- lower-R1 node n0024 (R1 0.082) is not a comparable model: space group P 1 differs from the delivered P -1 - a lower-symmetry trial refines more parameters again<br>- the deposited model re-refined on the delivered data reaches R1 0.0942 (deposited 0.0526): the deposited R1 is not reproducible on these reflections, so the del |
| 墙钟 | — | 12 min（718 s） |
| 并发泳道 | — | 0 |
| tokens | — | 8,086,799 |
| 工具调用 / 出错 | — | 156 / 2 |
...
### 3.1 nm-full-r1（B/full）

- rollout 记录 495 条；工具调用 156 次，其中出错 2 次；推理段 33 段

**工具错误率**

| 工具 | 调用 | 出错 | 错误率 | 累计耗时 | 最常见错误 |
|---|---|---|---|---|---|
| `run_shelxl` | 7 | 1 | 0.143 | 4.1s | `not run: 2 riding-H group(s) in this model carry an AFIX code SHELXL's connectivity check would refuse, and SH` |
| `view_structure` | 3 | 1 | 0.333 | 0.8s | `model has no atoms` |
| `exec` | 81 | 0 | 0.0 | — | — |
| `refine` | 6 | 0 | 0.0 | 5.9s | — |
| `validate_structure` | 6 | 0 | 0.0 | 0.2s | — |
| `situation_report` | 4 | 0 | 0.0 | 1.6s | — |
| `add_hydrogens` | 4 | 0 | 0.0 | 0.5s | — |
| `branch` | 4 | 0 | 0.0 | 1.8s | — |
| `model_disorder` | 4 | 0 | 0.0 | 0.5s | — |
| `audit_reflection_data` | 3 | 0 | 0.0 | 0.1s | — |
| `check_symmetry` | 3 | 0 | 0.0 | — | — |
| `compare_nodes` | 3 | 0 | 0.0 | 0.2s | — |
| `assemble_asu` | 3 | 0 | 0.0 | — | — |
| `get_project_brief` | 2 | 0 | 0.0 | 0.2s | — |
| `ingest_vendor_data` | 2 | 0 | 0.0 | 1.7s | — |
| `change_space_group` | 2 | 0 | 0.0 | 0.2s | — |
| `inspect_model` | 2 | 0 | 0.0 | — | — |
| `audit_element_assignment` | 2 | 0 | 0.0 | — | — |
| `set_restraints` | 2 | 0 | 0.0 | 0.3s | — |
| `checkout` | 2 | 0 | 0.0 | 0.3s | — |
...
| `write_outputs` | 1 | 0 | 0.0 | 0.3s | — |
| `run_checkcif` | 1 | 0 | 0.0 | 4.1s | — |
...
- `run_shelxl` 重复 1 次　参数 `{"l_s": 15, "mode": "adopt", "timeout_s": 300}`
- `assemble_asu` 重复 1 次　参数 `{}`
- `check_symmetry` 重复 1 次　参数 `{"timeout_s": 120}`
...
- 触发词 `instead of`，其前无工具报错
  > **Considering splitting atoms**  I think I need to split C00J and C00D together instead of just focusing on a single inconsistency. It might be better to undo and then apply model disorder to the atoms ["C00J", "C00D"], while sharing the FV…
...
| 工具 | 调用 | 出错 | 错误率 | 出错的格数 |
|---|---|---|---|---|
| `view_structure` | 3 | 1 | 0.333 | 1/1 |
| `run_shelxl` | 7 | 1 | 0.143 | 1/1 |
...
| `run_shelxt` | 1 | 2.3s |
| `branch` | 4 | 1.8s |
| `ingest_vendor_data` | 2 | 1.7s |
| `situation_report` | 4 | 1.6s |
| `view_structure` | 3 | 0.8s |
| `add_hydrogens` | 4 | 0.5s |
| `model_disorder` | 4 | 0.5s |
| `set_restraints` | 2 | 0.3s |
| `checkout` | 2 | 0.3s |
| `write_outputs` | 1 | 0.3s |
| `validate_structure` | 6 | 0.2s |
| `compare_nodes` | 3 | 0.2s |
...
|---|---|---|---|---|---|---|---|---|
| `run_shelxl` | 2 | 1 | 0 | 0 | 1 | 7 | 0 | 0.143 |
| `situation_report` | 2 | 0 | 0 | 0 | 2 | 4 | 0 | 0.0 |
| `validate_structure` | 1 | 0 | 0 | 0 | 1 | 6 | 0 | 0.0 |
| `view_structure` | 1 | 1 | 0 | 0 | 0 | 3 | 0 | 0.333 |
| `check_symmetry` | 1 | 0 | 0 | 0 | 1 | 3 | 0 | 0.0 |
| `assemble_asu` | 1 | 0 | 0 | 0 | 1 | 3 | 0 | 0.0 |
